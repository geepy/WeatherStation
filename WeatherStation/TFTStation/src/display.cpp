...
TftDisplay::TftDisplay() {
	_display = TFT_eSPI(TFT_WIDTH, TFT_HEIGHT);
	display = &_display;
...
void TftDisplay::drawRect(int x, int y, int w, int h) {
	display->drawRect(x, y, h, w, TFT_BLACK);
	display->drawRect(x + 1, y + 1, h - 2, w - 2, TFT_WHITE);
	display->drawRect(x + 2, y + 2, h - 4, w - 4, TFT_BLACK);
}

void TftDisplay::drawFrames() {
	display->drawRect(0, 0, TFT_WIDTH, TFT_HEIGHT, TFT_BLACK);

	// oben links
	display->drawRect(110, 0, 66, 76, TFT_BLACK);
	// oben mitte
	display->drawRect(110, 75, 66, 114, TFT_BLACK);
	// oben rechts
	display->drawRect(110, 188, 66, 76, TFT_BLACK);
	// mitte links
	display->drawRect(0, 0, 111, 88, TFT_BLACK);
	// mitte mitte
	display->drawRect(0, 87, 111, 90, TFT_BLACK);
	// mitte rechts
	display->drawRect(0, 176, 111, 88, TFT_BLACK);
}
...
void TftDisplay::drawTimeAndDate(date_time_t date) {
	display->fillRect(111, 76, 64, 112, TFT_WHITE);
...
void TftDisplay::drawSensorData(int index, SensorData& data)
{
	...
	int border = w - 24;
	display->fillRect(y1, x1, h, w, TFT_WHITE);
	display->drawBitmap(y1 + 84, x1 + 2, Bmps[index], 16, 16, TFT_BLACK);
